...
export default function Catalog() {
  ...
  // Local filter — NOT navigation, just a UI chip. Stays as React state.
  ...
  // ── Handle incoming deep-link from Home (location.state.openProductId) ──
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          // Navigate to: ?sub=<subCat>&product=<id>
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Exclusive products shared to public catalogue
  ...
  const goToCatalogueRoot = () => {
    navigate({ search: '' }); // clears all params → /catalogue
  };
